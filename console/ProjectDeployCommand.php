...
class ProjectDeployCommand extends RemoteCommand
{
    ...
    {
        if (!empty($this->backup['permissions']['root_user'])) {
            $this->question('Handling file ownership.');
            $this->sshRunAndPrint([$this->sudo . 'chown ' . $this->backup['permissions']['root_user'] . ' -R .']);
            $this->line('');
        }

        if (array_get($this->backup, 'branch_main', 'main') === false) {
    ...
    public function mergeDeploy()
    {
        ...
        $result = $this->sshRunAndPrint([
            'git fetch',
            'git merge origin/' . array_get($this->backup, 'branch_main', 'main'),
        ]);
        ...
        $this->sshRunAndPrint(['git push origin ' . $this->backup['branch']]);
    ...
    {
        if (empty($this->backup['permissions']['web_user']) || empty($this->backup['permissions']['web_folders'])) {
            return;
        }

        $folders = explode(',', $this->backup['permissions']['web_folders']);

        $this->question('Handling file ownership.');
        $this->line('');

        foreach ($folders as $folder) {
            $this->sshRunAndPrint([$this->sudo . 'sudo chown ' . $this->backup['permissions']['web_user'] . ' ' . $folder . ' -R']);
        }
    }
